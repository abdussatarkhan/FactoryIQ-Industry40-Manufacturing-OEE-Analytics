...
# 📊 FactoryIQ: Smart Factory & Industry 4.0 Equipment Effectiveness (OEE) Analytics

**Smart Manufacturing & Industry 4.0 Data Warehouse: Overall Equipment Effectiveness (OEE = Availability × Performance × Quality), MTBF failure analysis, and sensor anomaly detection.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/FactoryIQ-Industry40-Manufacturing-OEE-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/FactoryIQ-Industry40-Manufacturing-OEE-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/FactoryIQ-Industry40-Manufacturing-OEE-Analytics)
...
**FactoryIQ-Industry40-Manufacturing-OEE-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **Smart Manufacturing & Industrial IoT**.
...
### 1️⃣ Executive KPI Command Center (Industrial Dark Steel & Safety Amber (Dark))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_machines_cnc │──►│     fact_production_runs │◄──┤     dim_production_lines │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_operator_shifts │   │     dim_parts_catalog   │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM factoryiq_dw.fact_production_runs f
JOIN factoryiq_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/FactoryIQ-Industry40-Manufacturing-OEE-Analytics.git
cd FactoryIQ-Industry40-Manufacturing-OEE-Analytics
